...
public class PageFactory {
	...
	static {
		getClassInPackage(new com.cucumber.utilities.PropertiesFile()
				.getproperties("page.packages"));//go in application.properties and get the file
	}
		...
		try {
			...
			if (packageName != null) {
				String pageClassFullName = packageName + "."
						+ formattedPageName;
				page = (BasePage) Class.forName(pageClassFullName)//once you get the file(page)it takes full path of file
						//and creating new instance of page,we can pass any page,so page become that passing page
						.newInstance();
				//page = new LoginPage();
